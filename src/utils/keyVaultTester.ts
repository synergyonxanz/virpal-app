...
export class KeyVaultTester {
  ...
  private static async testSecretAccess(secretName: string): Promise<TestResult> {
    const startTime = performance.now();
    ...
    try {
      ...
      if (secret === null) {
        return {
          ...
          status: 'WARN',
          message: 'Secret not found or access denied (using fallback if available)',
          duration
        };
      }
      ...
      return {
        test: `Secret Access: ${secretName}`,
        status: 'FAIL',
        message: `Failed to retrieve secret: ${error instanceof Error ? error.message : 'Unknown error'}`,
        duration: performance.now() - startTime
      };
    }
  }
  ...
  private static async testInvalidSecret(): Promise<TestResult> {
    ...
    try {
      const secret = await frontendKeyVaultService.getSecret('invalid-secret-name-test');
      const duration = performance.now() - startTime;
      ...
      return {
        test: 'Invalid Secret Handling',
        status: 'PASS',
        message: `Invalid secret handled with error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        duration: performance.now() - startTime
      };
    }
  }
    ...
    try {
      ...
      if (initialMetrics.state === 'closed' || initialMetrics.state === 'open' || initialMetrics.state === 'half-open') {
        return {
        ...
        return {
          test: 'Circuit Breaker Functionality',
          status: 'WARN',
          message: 'Circuit breaker state is unknown or not properly initialized',
          duration: performance.now() - startTime
        };
      }
    } catch (error) {
  ...
  private static async testCircuitBreakerRecovery(): Promise<TestResult> {
    ...
    try {
      ...
      console.log(`🔄 Circuit breaker reset: ${beforeReset.failureCount} -> ${afterReset.failureCount} failures`);
  ...
  private static displayResults(results: TestResult[], summary: any, overall: string): void {
    ...
    console.log('\n' + '='.repeat(80));
    console.log(`Overall Status: ${overall === 'PASS' ? '✅ PASS' : 
                                   overall === 'WARN' ? '⚠️ WARNINGS' : '❌ FAIL'}`);
    console.log(`Total Tests: ${summary.total} | Passed: ${summary.passed} | Failed: ${summary.failed} | Warnings: ${summary.warnings}`);
    
    if (overall !== 'PASS') {
      console.log('\n🔧 Troubleshooting Tips:');
      
      if (summary.failed > 0) {
        console.log('• Check if Azure Functions is running: npm run watch (functions)');
        console.log('• Verify Azure Function endpoint in environment variables');
      ...
      if (summary.warnings > 0) {
        ...
        console.log('• Review authentication requirements for production secrets');
      }
    } else {
  ...
  static async testSecret(secretName: string): Promise<void> {
    ...
    try {
      ...
      if (secret === null) {
        ...
        console.log(`✅ Secret '${secretName}' retrieved successfully (length: ${secret.length}, ${duration.toFixed(0)}ms)`);
      }
    } catch (error) {
      console.log(`❌ Error retrieving secret '${secretName}': ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
    /**
  ...
  static async testCircuitBreakerBehavior(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log(`✅ Secret access result: ${secret ? 'Found' : 'Not found'}`);
        console.log(`Circuit state after success: ${metrics.state}, Failures: ${metrics.failureCount}`);
      } catch (error) {
        console.log(`⚠️ Secret access failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        metrics = frontendKeyVaultService.getCircuitBreakerMetrics();
        console.log(`Circuit state after failure: ${metrics.state}, Failures: ${metrics.failureCount}`);
      }
      ...
      for (let i = 1; i <= 4; i++) {
        ...
        const currentMetrics = frontendKeyVaultService.getCircuitBreakerMetrics();
        console.log(`  State: ${currentMetrics.state}, Failures: ${currentMetrics.failureCount}`);
        
        if (currentMetrics.state === 'open') {
          console.log(`🔴 Circuit breaker opened after ${currentMetrics.failureCount} failures!`);
          break;
        }
      }
      ...
      if (finalMetrics.state === 'open') {        try {
          await frontendKeyVaultService.getSecret('test-secret-blocked');
          console.log(`⚠️ Unexpected: Request succeeded when circuit should be open`);
        } catch (error) {
          if (error instanceof Error && error.message.includes('circuit breaker is open')) {
            console.log(`✅ Circuit breaker correctly blocked request: ${error.message}`);
          } else {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            console.log(`⚠️ Request failed but not due to circuit breaker: ${errorMessage}`);
          }
        }
      }
      ...
      console.log(`✅ Circuit breaker reset: State=${resetMetrics.state}, Failures=${resetMetrics.failureCount}`);
   ...
   */
  static startCircuitBreakerMonitoring(intervalMs: number = 5000): NodeJS.Timeout {
    console.log(`📊 Starting circuit breaker monitoring (${intervalMs}ms intervals)...`);
