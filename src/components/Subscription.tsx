...
const Subscription: React.FC = () => {
  ...
  const handleSubscribe = (planName: string) => {
    ...
    alert(
      `Terima kasih! Fitur langganan untuk paket ${planName} akan segera hadir. Anda akan diarahkan ke halaman pembayaran.`
    );
  };
    ...
    <div className="space-y-8 p-4 sm:p-6 md:p-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-lg leading-relaxed" style={virpalNeutralDarkColor}>
          Tingkatkan pengalaman VIRPAL Anda dengan fitur-fitur premium yang
        ...
        {plans.map((plan, index) => (
          ...
          >
            ...
            <div className="text-center mb-6">
              ...
              {/* Ukuran emoji lebih besar */}
              <h4
                className="text-2xl font-bold mb-2"
                style={virpalPrimaryColor}
              >
                {plan.name}
              </h4>
              <div className="mb-4">
                ...
                {plan.period && (
                  <span className="text-base" style={virpalNeutralDarkColor}>
                    {' '}
                    {/* Ukuran teks periode lebih besar */}
                    {plan.period}
                  </span>
                )}
              </div>
            ...
            <ul className="space-y-3 mb-6 flex-grow">
              {' '}
              {/* flex-grow agar tombol selalu di bawah */}
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-lg font-semibold transition-all duration-200 mt-auto ${
                plan.isPopular ? 'text-white shadow-lg' : 'border-2'
              }`}
              style={{
                backgroundColor: plan.isPopular
                  ? 'var(--virpal-primary)'
              ...
              onMouseEnter={(e) => {
                if (!e.currentTarget.disabled) {
                  e.currentTarget.style.backgroundColor = plan.isPopular
                    ...
                    : 'var(--virpal-accent)'; // Ganti hover dengan secondary/accent
                  e.currentTarget.style.color = plan.isPopular
                    ? 'white'
                    : 'var(--virpal-primary)';
                  if (!plan.isPopular)
                    e.currentTarget.style.borderColor = 'var(--virpal-primary)'; // Border tetap
                }
              }}
              onMouseLeave={(e) => {
                if (!e.currentTarget.disabled) {
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-sm" style={virpalNeutralDarkColor}>
              Anda dapat mengelola langganan (upgrade, downgrade, atau
              pembatalan) kapan saja melalui halaman pengaturan akun Anda. Fitur
              premium akan tetap aktif hingga akhir periode billing Anda.
            </p>
          </div>
          ...
          >
            ...
            <p className="text-sm" style={virpalNeutralDarkColor}>
              Ya, privasi dan keamanan data Anda adalah prioritas utama kami.
              Semua percakapan Anda dienkripsi end-to-end. Untuk paket gratis,
              riwayat tersimpan lokal di perangkat Anda. Untuk paket
              Premium/Pro, riwayat disimpan di server aman kami dengan protokol
              enkripsi terdepan. Kami tidak pernah membagikan data pribadi Anda
              kepada pihak ketiga.
            </p>
          </div>
          ...
          >
            ...
            <p className="text-sm" style={virpalNeutralDarkColor}>
              VIRPAL dirancang sebagai pendamping emosional dan alat dukungan
              untuk kesejahteraan mental sehari-hari. Kami bukan pengganti
              terapi, diagnosis, atau penanganan medis dari profesional. Untuk
              kondisi kesehatan mental yang serius, kami sangat menyarankan
              untuk berkonsultasi dengan psikolog atau psikiater berlisensi.
            </p>
          </div>
          ...
          >
            ...
            <p className="text-sm" style={virpalNeutralDarkColor}>
              Paket Premium menawarkan pengalaman VIRPAL tanpa batas dengan
              percakapan lebih mendalam, riwayat percakapan tak terbatas yang
              disimpan di server, pilihan karakter AI yang lebih beragam, dan
              fitur-fitur eksklusif lainnya yang dirancang untuk dukungan
              emosional yang lebih personal dan komprehensif.
            </p>
          </div>
        </div>
      </div>
