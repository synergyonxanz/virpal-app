...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  useEffect(() => {
    ...
    >{/* Header dengan Logo dan tombol tutup */}
      <div
        className="p-4 flex items-center justify-between theme-transition"
      ...
      <div className="p-4 flex flex-col gap-2 theme-transition">        <button
          onClick={onNewChat}
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.backgroundColor = 'var(--virpal-sidebar-hover)';
            e.currentTarget.style.cursor = 'pointer';
          }}
          onMouseLeave={(e) => {
        ...
        >          <span 
          ...
          className={`w-full flex items-center gap-3 rounded-lg p-3 font-semibold text-base transition-colors theme-transition`}
          style={{
            backgroundColor: isHistoryOpen ? 'var(--virpal-accent)' : 'var(--virpal-sidebar-bg)',
            color: isHistoryOpen ? 'var(--virpal-primary)' : 'var(--virpal-sidebar-text)',
            cursor: 'pointer'
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.cursor = 'pointer';
          ...
          onMouseUp={(e) => {
            ...
            className="w-8 h-8 rounded flex items-center justify-center text-xl theme-transition"
            style={{
              backgroundColor: isHistoryOpen ? 'var(--virpal-primary)' : 'var(--virpal-neutral-lighter)',
      ...
      </div>      {/* Settings Button - hanya tampil saat sidebar terbuka */}
      <div className="mt-auto p-4 relative theme-transition" ref={settingsRef}>        {/* Dropup Menu */}
        {(isSettingsOpen || isSettingsClosing) && (          <div 
            ...
            style={{ 
              ...
              animation: isSettingsClosing ? 'slideOutToBottom 0.2s cubic-bezier(0.4, 0, 0.2, 1)' : 'slideInFromBottom 0.3s cubic-bezier(0.16, 1, 0.3, 1)',
              transformOrigin: 'bottom center'
            }}
          >            <div className="p-2 relative theme-transition">
              {/* Tema dengan container yang extended untuk menutupi gap */}              <div 
                ref={themeAreaRef}
              ...
              >                <div 
                ...
                {showThemeOptions && !isSettingsClosing && (
                  <div 
                    data-submenu="theme"                    className="absolute left-56 -top-2 min-w-32 rounded-lg shadow-lg border z-60 theme-transition"
                    style={{ 
                      backgroundColor: 'var(--virpal-sidebar-bg)',
                      borderColor: 'var(--virpal-neutral-lighter)',
                    ...
                    <div className="p-1">
                      {themeOptions.map((option, index) => (
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded text-sm transition-all duration-200 flex items-center justify-between theme-transition"                          style={{
                            backgroundColor: currentTheme === option.label ? 'var(--virpal-accent)' : 'transparent',
                            color: currentTheme === option.label ? 'var(--virpal-primary)' : 'var(--virpal-sidebar-text)',
                            cursor: 'pointer',
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 rounded-lg p-3 font-semibold text-base transition-colors theme-transition`}
          style={{
            backgroundColor: isSettingsOpen ? 'var(--virpal-accent)' : 'var(--virpal-sidebar-bg)',
            color: isSettingsOpen ? 'var(--virpal-primary)' : 'var(--virpal-sidebar-text)',
            cursor: 'pointer'
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.cursor = 'pointer';
        ...
        >
          <span 
            className="w-8 h-8 rounded flex items-center justify-center text-xl theme-transition"
            style={{
              backgroundColor: isSettingsOpen ? 'var(--virpal-primary)' : 'var(--virpal-neutral-lighter)',
