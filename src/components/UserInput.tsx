...
const UserInput: React.FC<UserInputProps> = ({ 
  ...
  const [selectedMode, setSelectedMode] = useState<'default' | 'todo' | 'help'>('default');

  // Guest limitations
  const canSendMessage = isAuthenticated || guestLimitService.canSendMessage();
  const remainingMessages = isAuthenticated ? null : guestLimitService.getRemainingMessages();
  const warningMessage = isAuthenticated ? null : guestLimitService.getWarningMessage();
    ...
    <div>
      ...
      {!isAuthenticated && (
        <div className="mb-2 p-3 rounded-lg border" style={{
          backgroundColor: remainingMessages === 0 ? 'var(--virpal-error-bg)' : 'var(--virpal-warning-bg)',
          borderColor: remainingMessages === 0 ? 'var(--virpal-error)' : 'var(--virpal-warning)',
          color: remainingMessages === 0 ? 'var(--virpal-error)' : 'var(--virpal-warning-text)'
        }}>
      ...
      <div className="relative rounded-2xl shadow-xl border border-[var(--virpal-primary_opacity_30,rgba(121,80,242,0.3))] theme-transition" style={{ backgroundColor: 'var(--virpal-content-bg)' }}>
      {/* Input area with all buttons inside */}
      <form onSubmit={handleSubmit} className="relative p-3 theme-transition">        <textarea
          value={inputText}
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between theme-transition">          {/* Left side buttons */}
          <div className="flex gap-2 theme-transition">            <button 
              ...
              className={`px-3 py-1.5 text-xs rounded-lg font-medium shadow-sm transition-all duration-200 cursor-pointer theme-transition ${
                ...
                backgroundColor: selectedMode === 'todo' ? 'var(--virpal-primary)' : 'var(--virpal-content-bg)',
                borderColor: selectedMode === 'todo' ? 'var(--virpal-primary)' : 'var(--virpal-neutral-lighter)',
                color: selectedMode === 'todo' ? 'white' : 'var(--virpal-neutral-default)',
                cursor: 'pointer'
              }}
              onMouseEnter={(e) => {
                ...
                if (selectedMode !== 'todo') {
                  ...
                  e.currentTarget.style.color = 'var(--virpal-neutral-default)';
                }
              }}
              onMouseLeave={(e) => {
                if (selectedMode !== 'todo') {
                  e.currentTarget.style.backgroundColor = 'var(--virpal-content-bg)';
                  e.currentTarget.style.color = 'var(--virpal-neutral-default)';
                }
              }}
              onMouseDown={(e) => {
            ...
            >
              ...
              className={`px-3 py-1.5 text-xs rounded-lg font-medium shadow-sm transition-all duration-200 cursor-pointer theme-transition ${
                ...
                backgroundColor: selectedMode === 'help' ? 'var(--virpal-primary)' : 'var(--virpal-content-bg)',
                borderColor: selectedMode === 'help' ? 'var(--virpal-primary)' : 'var(--virpal-neutral-lighter)',
                color: selectedMode === 'help' ? 'white' : 'var(--virpal-neutral-default)',
                cursor: 'pointer'
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.cursor = 'pointer';
          ...
          {/* Right side send button */}          <button
            ...
            style={{
              backgroundColor: 'var(--virpal-primary)',
              cursor: (isSending || !inputText.trim() || !canSendMessage) ? 'not-allowed' : 'pointer'
            }}
            onMouseEnter={(e) => {
