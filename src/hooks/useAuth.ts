...
import type { AuthState, UserProfile, TokenInfo, ExtendedAuthResult } from '../services/authService';
...
const getErrorMessage = (errorMessage: string): string => {
  if (errorMessage.includes('user_cancelled') || errorMessage.includes('PopupHandler.monitorPopupForHash')) {
...
export interface UseAuthOptions {
  onAuthStateChange?: (isAuthenticated: boolean, user: UserProfile | null) => void;
  autoLogin?: boolean;
...
export function useAuth(options: UseAuthOptions = {}): UseAuthReturn {
  ...
  }, [safeSetAuthState, silentTokenRenewal, tokenRefreshThreshold, setupTokenRefresh]);  /**
   * Login dengan popup
   */
  const login = useCallback(async (): Promise<ExtendedAuthResult> => {
    try {
      ...
      return result;
    } catch (error) {      // Handle MSAL popup errors more intelligently
  ...
  }, [safeSetAuthState, handleError, silentTokenRenewal, tokenRefreshThreshold, setupTokenRefresh, onAuthStateChange]);
   ...
   */
  const getAccessToken = useCallback(async (forceRefresh: boolean = false): Promise<string> => {
    try {
      ...
      handleError(error instanceof Error ? error : 'Failed to get access token');
      throw error;
    }
  ...
  useEffect(() => {
    ...
    const initTimeout = setTimeout(() => {
      if (!isInitialized) {
        setIsInitialized(true);
        updateAuthState({ isLoading: false, error: 'Authentication initialization timed out' });
      }
